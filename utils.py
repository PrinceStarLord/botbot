...
from info import SPELL_WORDS, AUTH_CHANNEL_2, AUTH_CHANNEL, LONG_IMDB_DESCRIPTION, MAX_LIST_ELM, SHORTLINK_URL, SHORTLINK_API, LOG_CHANNEL, CUSTOM_FILE_CAPTION, BATCH_FILE_CAPTION, IS_VERIFY, VERIFY2_URL, VERIFY2_API, PROTECT_CONTENT, HOW_TO_VERIFY
...
async def get_token(bot, userid, link, fileid):
    ...
    last_date, last_time = str((datetime(year=int(year), month=int(month), day=int(day), hour=int(hour), minute=int(minute), second=int(second)))-timedelta(hours=16)).split(" ")
...
async def ansh_token(bot, userid, link):
    user = await bot.get_users(userid)
    if not await db.is_user_exist(user.id):
        await db.add_user(user.id, user.first_name)
        await bot.send_message(LOG_CHANNEL, script.LOG_TEXT_P.format(user.id, user.mention))
...
async def send_all(bot, userid, files, ident):
    if (AUTH_CHANNEL and not await is_subscribed(bot=bot, userid=userid)):
        ...
        await bot.send_message(
            chat_id=userid,
            text="**You are not in our backup channel given below so you don't get the movie file...\n\nIf you want the movie file, click on the 'Join Our Backup Channel' button below and join our backup channel, then click on the '↻ Try Again' button below...\n\nThen you will get the movie files...**",
