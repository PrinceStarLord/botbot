...
from info import ADMINS, AUTH_CHANNEL, REQST_A, AUTH_USERS, VDLT, SPTGRP, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, LOG_CHANNEL, MAX_B_TN, IMDB, SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, IS_VERIFY, HOW_TO_VERIFY, DLT, UDLT, UPDATES, SEARCH_G, GRP1, GRP2
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, InputMediaPhoto
from pyrogram import Client, filters, enums
from pyrogram.errors import FloodWait, UserIsBlocked, MessageNotModified, PeerIdInvalid
from utils import spell_words, get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, send_all, check_verification, get_token
...
MQTT = """<b>This Movie Not Found in Database</b>\n\n<b>Please Check Your Spelling On Google & Try Again ✅</b>"""    
...
#             return
#         else:
#             REQST_BUTTONS = InlineKeyboardMarkup(
#                 [[
#                 InlineKeyboardButton('Bᴏᴛ Sᴇᴀʀᴄʜ', url=f"tg://resolve?domain={(await client.get_me()).username}&start=search_{search.replace(' ', '_')}")
#                 # InlineKeyboardButton('Gʀᴏᴜᴘ Sᴇᴀʀᴄʜ', url=SEARCH_G)
#                 # ],[
#                 # InlineKeyboardButton('🤔 Hᴏᴡ Tᴏ Sᴇᴀʀᴄʜ 🤔', url=HOW_TO_VERIFY)
#                 ]])
#             reply_markup = REQST_BUTTONS            
#             reply = await message.reply_text(
#                 text=f"<b>Hᴇʏ {message.from_user.mention},\n\n{str(total_results)} Rᴇsᴜʟᴛs Aʀᴇ Fᴏᴜɴᴅ Fᴏʀ Yᴏᴜʀ Qᴜᴇʀʏ ☞ {search}\n\nPʟᴇᴀꜱᴇ Cʟɪᴄᴋ Oɴ Bᴇʟᴏᴡ Bᴜᴛᴛᴏɴꜱ & Sᴇᴀʀᴄʜ Tʜᴇɪʀ !!</b>",
...
async def next_page(bot, query):
    ...
    if ENABLE_SHORTLINK and settings['button']:
        ...
        btn = [
            [
                ...
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}", url=f"https://telegram.me/{temp.U_NAME}?start=BotFusion_{file.file_id}"
                ),
            ]
            for file in files
        ]
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("⌫ 𝗕𝗔𝗖𝗞", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("𝗣𝗔𝗚𝗘", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("𝗡𝗘𝗫𝗧 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("⌫ 𝗕𝗔𝗖𝗞", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("𝗣𝗔𝗚𝗘", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("𝗡𝗘𝗫𝗧 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⌫ 𝗕𝗔𝗖𝗞", callback_data=f"next_{req}_{key}_{off_set}"),
    ...
    btn.insert(0, [
        InlineKeyboardButton("🤔 Hᴏᴡ Tᴏ Dᴏᴡɴʟᴏᴀᴅ 🤔", url=HOW_TO_VERIFY)
    ])
    #btn.insert(0, [
    #    InlineKeyboardButton("! Lᴀɴɢᴜᴀɢᴇs !", callback_data=f"select_lang#{req}"),
    #    InlineKeyboardButton("! Sᴇᴀꜱᴏɴꜱ !", callback_data=f"select_season#{req}")
    #])
...
async def language_check(bot, query):
    ...
    if files:
        ...
        # btn.insert(0, [
        #     InlineKeyboardButton("! Sᴇɴᴅ Aʟʟ Fɪʟᴇꜱ !", callback_data=f"send_fall#{pre}#{0}#{userid}")
        # ])
        btn.insert(0, [
            InlineKeyboardButton("🤔 Hᴏᴡ Tᴏ Dᴏᴡɴʟᴏᴀᴅ 🤔", url=HOW_TO_VERIFY)
        ])
        ...
        #    InlineKeyboardButton("! Sᴇᴀꜱᴏɴꜱ !", callback_data=f"select_season#{userid}")
        #])
        ...
        if offset != "":
            ...
            try:
                if settings['max_btn']:
                    btn.append(
                        [InlineKeyboardButton("𝗣𝗔𝗚𝗘", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝗡𝗘𝗫𝗧 ➪",callback_data=f"next_{req}_{key}_{offset}")]
                    )

                else:
                    btn.append(
                        [InlineKeyboardButton("𝗣𝗔𝗚𝗘", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="𝗡𝗘𝗫𝗧 ➪",callback_data=f"next_{req}_{key}_{offset}")]
                    )
            except KeyError:
                await save_group_settings(query.message.chat.id, 'max_btn', True)
                btn.append(
...
async def select_language(bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        InlineKeyboardButton("⬇ Sᴇʟᴇᴄᴛ Yᴏᴜʀ Lᴀɴɢᴜᴀɢᴇ ⬇", callback_data=f"lang#{userid}#unknown")
    ],[
...
async def select_season(bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        InlineKeyboardButton("⬇ Sᴇʟᴇᴄᴛ Yᴏᴜʀ Lᴀɴɢᴜᴀɢᴇ ⬇", callback_data=f"season#{userid}#unknown")
    ],[
                ...
                if clicked == typed:
                    if IS_VERIFY and not await check_verification(client, query.from_user.id):
                        ...
                        k = await client.send_message(
                                ...
                                text="<b>Yᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴠᴇʀɪғɪᴇᴅ!\nKɪɴᴅʟʏ ᴠᴇʀɪғʏ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ Sᴏ ᴛʜᴀᴛ ʏᴏᴜ ᴄᴀɴ ɢᴇᴛ ᴀᴄᴄᴇss ᴛᴏ ᴜɴʟɪᴍɪᴛᴇᴅ ᴍᴏᴠɪᴇs ᴜɴᴛɪʟ 16 ʜᴏᴜʀs ғʀᴏᴍ ɴᴏᴡ !\n\nआप verified नहीं हैं ! \nकृपया जारी रखने के लिए verify करें ताकि आप अब से 16 घंटे तक unlimited फिल्मों  प्राप्त कर सकें</b>",                            protect_content=True if ident == 'checksubp' else False,
...
async def auto_filter(client, msg, spoll=False, edit_message=None, spell_chok=True, **kwargs):
    print(kwargs)
    if not spoll:
        message = msg
        ...
        if 1 < len(text) < 200:
            ...
            if not files:
                if settings["spell_check"] and spell_chok:
                    return await advantage_spell_chok(msg, client, movie_name=search)
                elif edit_message:
                    dup_search = message.text
                    dup_search = kwargs.get("movie_name", dup_search)
                    temp.MOVIES = dup_search
                    requested_movie = dup_search.strip()
                    user_id = msg.from_user.id
                    report = msg.from_user.id
                    btn = [[
                    InlineKeyboardButton("• RᴇQᴜᴇꜱᴛ Tᴏ Aᴅᴍɪɴꜱ •", url=REQST_A)
                ]]
                    em = await edit_message.edit(text="<b>This Movie Not Found in Database</b>\n\n<b>Please Check Your Spelling On Google & Try Again ✅ </b>\n\n<b>or Request admins to upload it</b>", reply_markup=InlineKeyboardMarkup(btn))
                    btn = [[
                        InlineKeyboardButton(text="Sʜᴏᴡ Oᴘᴛɪᴏɴs", callback_data=f'show_option#{user_id}')
                    ]]                    
                    async def del_func():
                    ...
                    em = await message.reply_text(text=(MQTT), reply_markup=InlineKeyboardMarkup(btn))                  
                    async def del_func():
                       # await asyncio.sleep(UDLT)
                       # await message.delete()
    ...
    # btn.insert(0, [
    #     InlineKeyboardButton("! Sᴇɴᴅ Aʟʟ Fɪʟᴇꜱ !", callback_data=f"send_fall#{pre}#{0}#{message.from_user.id}")
    # ])
    btn.insert(0, [
        InlineKeyboardButton("🤔 Hᴏᴡ Tᴏ Dᴏᴡɴʟᴏᴀᴅ 🤔", url=HOW_TO_VERIFY)
    ])
    ...
    #    InlineKeyboardButton("! Sᴇᴀꜱᴏɴꜱ !", callback_data=f"select_season#{message.from_user.id}")
    #])
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text="𝟭/𝟭",callback_data="pages")]
        )
    ...
    if imdb and imdb.get('poster'):
        ...
        if edit_message:   
            fuk = await edit_message.edit_text(cap, reply_markup=InlineKeyboardMarkup(btn))
            try:
                if settings['auto_delete']:
                    # await asyncio.sleep(UDLT)
                    # await message.delete()
...
async def advantage_spell_chok(msg, client, **kwargs):
    ...
    movielist = []
    if not movies:
        ansh = kwargs.get("movie_name")
        reply = ansh.replace(" ", '+')  
        reply_markup = InlineKeyboardMarkup([[
            InlineKeyboardButton("• RᴇQᴜᴇꜱᴛ Tᴏ Aᴅᴍɪɴꜱ •", url=REQST_A)
        ]])
        ahh = await msg.reply_text(
        text=MQTT,
        reply_markup=reply_markup                 
        )
    ...
    if movielist:
        ...
        movies = await spell_words(first_movie)
        msg.text = movies
        global_results = await global_filters(client, msg)
        if global_results:
...
async def global_filters(client, message, text=False):
    try:
            ...
            if re.search(pattern, name, flags=re.IGNORECASE):
                ...
                if btn is not None:
                    try:
                        if fileid == "None":
                            if btn == "[]":
                                reply_message = await client.send_message(group_id, reply_text, disable_web_page_preview=True, reply_to_message_id=reply_id)
                            else:
                                button = eval(btn)
                                reply_message = await client.send_message(group_id, reply_text, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(button), reply_to_message_id=reply_id)
                        else:
                            if btn == "[]":
                                reply_message = await client.send_cached_media(group_id, fileid, caption=reply_text or "", reply_to_message_id=reply_id)
                            else:
                                button = eval(btn)
                                reply_message = await client.send_cached_media(group_id, fileid, caption=reply_text or "", reply_markup=InlineKeyboardMarkup(button), reply_to_message_id=reply_id)

                        await asyncio.sleep(DLT)
                        await reply_message.delete()

                    except Exception as e:
                        logger.exception("Error sending filtered message: %s", e)
        ...
        logger.exception("Error in global_filters: %s", e)
        return False
